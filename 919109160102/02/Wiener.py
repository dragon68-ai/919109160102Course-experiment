...
if __name__=='__main__':
    image = cv2.imread('./02/cameraman.tif')
    image = cv2.cvtColor(image,cv2.COLOR_BGR2GRAY)
    img_h=image.shape[0]
    img_w=image.shape[1]

    cv2.imwrite('./02/result/original.jpg',image)

    PSF = motion_process((img_h,img_w), 60)
    blurred = np.abs(make_blurred(image, PSF, 1e-3))
    cv2.imwrite('./02/result/Motion_blurred.jpg',blurred)
