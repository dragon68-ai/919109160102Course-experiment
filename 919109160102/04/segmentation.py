...
if __name__=='__main__':
    img = cv2.imread('./04/house.tif')
    img = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
    cv2.imwrite('./04/result/original.jpg',img)
    T1=otsu(img,-1)
    print(f"ostu_Best threshold = {T1}")
    T2=iterate(img,1,20)
    print(f"iterate_Best threshold = {T2}")
    seg_img1=segmenting(img,T1)
    cv2.imwrite('./04/result/seg_otsu_house.jpg',seg_img1)
    seg_img2=segmenting(img,T2)
    cv2.imwrite('./04/result/seg_iter_house.jpg',seg_img2)

    img1 = cv2.imread('./04/cameraman.tif')
    img1 = cv2.cvtColor(img1, cv2.COLOR_BGR2GRAY)
    cv2.imwrite('./04/result/original.jpg',img1)
    T1=otsu(img1,-1)
    print(f"ostu_Best threshold = {T1}")
    T2=iterate(img1,1,20)
    print(f"iterate_Best threshold = {T2}")
    seg_img1=segmenting(img1,T1)
    cv2.imwrite('./04/result/seg_otsu_cameraman.jpg',seg_img1)
    seg_img2=segmenting(img1,T2)
    cv2.imwrite('./04/result/seg_iter_cameraman.jpg',seg_img2)

# 多目标图片
    img2 = cv2.imread('./04/peppers_gray.tif')
    img2 = cv2.cvtColor(img2, cv2.COLOR_BGR2GRAY)
    cv2.imwrite('./04/result/original.jpg',img2)
    T1=otsu(img2,-1)
    print(f"ostu_Best threshold = {T1}")
    T2=iterate(img2,1,20)
    print(f"iterate_Best threshold = {T2}")
    seg_img1=segmenting(img2,T1)
    cv2.imwrite('./04/result/seg_otsu_peppers.jpg',seg_img1)
    seg_img2=segmenting(img2,T2)
    cv2.imwrite('./04/result/seg_iter_peppers.jpg',seg_img2)
